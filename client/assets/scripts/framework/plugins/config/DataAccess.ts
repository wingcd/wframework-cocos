...
type DataKeyMap<T> = {[key in KeyType]: Readonly<T>};
...
export class DataTable<T> extends DataItem<T> {
    ...
    private _itemMap: DataKeyMap<T>;
    public get itemMap(): Readonly<DataKeyMap<T>> {
    ...
    protected initDataAsDict(): DataKeyMap<T> {
        let itemMap: DataKeyMap<T> = {};
        try{
